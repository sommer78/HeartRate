...
* This file contains misc interaction 
*
**************************************************************************/
#include <stdint.h>
#include "config.h"
...
void notification_start( uint8_t type, uint16_t value )
{
    switch(type) {
        case NOTIFICATION_BONDING:
            led_action_control(NOTIFICATION_BONDING,SERIAL_FLASH,1000);
            break;
        ...
        case NOTIFICATION_CALLING:
            if(led_action_control(NOTIFICATION_CALLING,FIRST_LED_FLASH,6)) {
                motor_action_control_start(SLOW_VIBRATE,6);
            }
            break;
        case NOTIFICATION_ALARM:
            if(led_action_control(NOTIFICATION_ALARM,LED_BLANK,10)) {
