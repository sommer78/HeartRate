...
extern uint8_t global_send_header_wait_ack_complete;
extern uint8_t global_send_ack_wait_data_end_complete;
static void on_tx_complete(ble_nus_t * p_nus, ble_evt_t * p_ble_evt)
{
    ...
    if(global_send_ack_wait_data_end_complete == 1) {
        global_send_ack_wait_data_end_complete = 0;
    }

#ifdef DEBUG_LOG
    uint8_t size = p_ble_evt->evt.common_evt.params.tx_complete.count;
    LOG(LEVEL_INFO,"send %d package complete\r\n",size);
    //   char printBuffer[5];
    //   sprintf(printBuffer,"%d",size);
    //   simple_uart_putstring((const uint8_t *)"send ");
    //   simple_uart_putstring((const uint8_t *)printBuffer);
    //   simple_uart_putstring((const uint8_t *)" package complete\r\n");
#endif
...
uint32_t ble_nus_send_string(ble_nus_t * p_nus, uint8_t * string, uint16_t *length)
{
    ...
    if(string[0] == 0xAB && string[1] == 0x10) {
        ...
        //    simple_uart_putstring((const uint8_t *)"res pack & con is:");
    } else {
        LOG(LEVEL_INFO,"data pack & con is:%s\r\n",dumpContent);
        //    simple_uart_putstring((const uint8_t *)"data pack & con is:");
    }

    //    simple_uart_putstring((const uint8_t *)dumpContent);
