...
extern float totalDistance,totalCalory;
...
{
    return ((mSportHead.length != 0) && (mSportsData.bits.steps != 0));
}
...
static void send_sport_flash_cb(void)
{
...
static void send_sleep_flash_cb(void)
{
...
void send_all_data(bool is_from_cb)
{
    ...
    }   else if( (false == is_from_cb && true == notify_steps_enable
                && have_sport_data_in_ram_to_send()) ||  have_history_sport_data_in_ram_to_send()) {
