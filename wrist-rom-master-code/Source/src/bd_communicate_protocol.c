...
* This file contains the bluetooth communicate protocol
*
**************************************************************************/
#include "stdint.h"
#include "string.h"
...
uint8_t global_send_header_wait_ack_complete = 0;
uint8_t global_send_ack_wait_data_end_complete = 0;
/****************************************************************************
* Tempory send buffer
*****************************************************************************/
...
static void delay_send_func(void * context)
{
    ...
    if(content->contentLeft == 0) { //send complete but not get ack
        ...
        if(content->resendCount < 3) {
            ...
            //    simple_uart_putstring((const uint8_t *)"time out resend \r\n");
#endif
    ...
    global_send_header_wait_ack_complete = 0;
    global_send_ack_wait_data_end_complete = 0;
...
static uint32_t L1_resend_package(L1_Send_Content * content)
{
    ...
    //simple_uart_putstring((const uint8_t *)"will resend a package\r\n");
#endif
...
static void response_package_handle(uint16_t sequence_id,uint8_t crc_check)
{
    ...
    if(!current_package_wait_response) {
        ...
        //    simple_uart_putstring((const uint8_t *)" no package wait res\r\n");
#endif
    ...
    if(current_package_wait_response->sequence_id == sequence_id ) {
        ...
        if( crc_check == CRC_SUCCESS) {
            ...
            //    simple_uart_putstring((const uint8_t *)"get response show crc succ\r\n");
#endif
            ...
            //    simple_uart_putstring((const uint8_t *)"get response show crc fail\r\n");
#endif
            ...
            if(current_package_wait_response->resendCount >= 3) {
                ...
                //    simple_uart_putstring((const uint8_t *)"response crc err resend\r\n");
#endif
        ...
        //    simple_uart_putstring((const uint8_t *)"receive a package with wrong sequesnce id\r\n");
    }
#endif
}
...
void schedule_async_send(void * contenxt,SEND_TASK_TYPE_T task)
{
    ...
    *************************************************************************/
    ...
    if(task == TASK_DATA) { //may be comes from data (send & resend) or (send complete callback)
        ...
        //   simple_uart_putstring(((const uint8_t *)"call schedule_async_send with wrong para\r\n"));
#endif
...
LABEL_SEND_ACK:
    if(current_task_type == TASK_ACK) {
        ...
        if(g_ack_package_buffer.isUsed == 1) {
            ...
            if(global_send_ack_wait_data_end_complete == 1) { //if data end not send complete
            ...
            if(error_code == NRF_SUCCESS) {
                ...
                global_send_header_wait_ack_complete = 1;
    ...
    while(error_code == NRF_SUCCESS) { // send will continue before buffer was full
        ...
        ************************************************************/
        ...
        if(L1_header_need_schedule.isUsed == 1) {
            //sending ack but not get response
            if(global_send_header_wait_ack_complete == 1) {
                sendPara.callback = schedule_async_send;
                ...
                LOG(LEVEL_INFO,"will wait ack send comple\r\n");
                //    simple_uart_putstring((const uint8_t *)"will wait ack send comple\r\n");
#endif
            ...
            switch(sendContentType) {
                ...
                case CONTENT_DATA:
                    ...
                    if(content ->contentLeft == 0) { //send complete & will register wait response
                        global_send_ack_wait_data_end_complete = 1;
                        sendPara.callback = NULL;
                        sendPara.context = NULL;
                        sendPara.task_type = TASK_NONE;
                        set_complete_callback(sendPara);
        ...
        if((sendContentType == CONTENT_HEADER) || (( CONTENT_DATA == sendContentType) && (content ->contentLeft != 0))) {
            sendPara.callback = schedule_async_send;
            sendPara.context = content;
            sendPara.task_type = TASK_DATA;
            set_complete_callback(sendPara);
...
void schedule_crc_error_handle(void * para,uint16_t event_size)
{
    ...
    //    simple_uart_putstring((const uint8_t *)"received data crc check error\r\n");
...
#ifdef DEBUG_LOG
        //simple_uart_putstring((const uint8_t *)"stop re timer\r\n");
#endif
    ...
    switch (receive_state) {
        case WAIT_START: // we need package start
            ...
            if(length_to_receive <= 0) { // just one package
                ...
                if(inner_version.version_def.ack_flag == RESPONSE_PACKAGE) { //response package
                    ...
                    //    simple_uart_putstring((const uint8_t *)"receive a ack package\r\n");
#endif
                ...
                if(L1_crc_check(crc16_value,received_buffer+L1_HEADER_SIZE,(received_buffer[L1_PAYLOAD_LENGTH_LOW_BYTE_POS] | (received_buffer[L1_PAYLOAD_LENGTH_HIGH_BYTE_POS] << 8))) == NRF_SUCCESS) { //check crc for received package
                    ...
                    //    simple_uart_putstring((const uint8_t *)"will send success response\r\n");
#endif
                    //send response
                    ...
                    //    simple_uart_putstring((const uint8_t *)"will send crc fail response\r\n");
#endif
                    //send response
        ...
        case WAIT_MESSAGE:
            ...
            if(length_to_receive <= 0) {
                ...
                /* Stop timer */
#ifdef DEBUG_LOG
                //simple_uart_putstring((const uint8_t *)"complete so stop re timer\r\n");
#endif
...
#ifdef DEBUG_LOG
                //simple_uart_putstring((const uint8_t *)"start re timer\r\n");
#endif
                app_timer_start(receive_time_out_timer,RECEIVE_TIMEOUT,&receive_state);
            }
        ...
        case MESSAGE_RESOLVE:
            ...
            if(inner_version.version_def.ack_flag == RESPONSE_PACKAGE) { //response package
                ...
                //     simple_uart_putstring((const uint8_t *)"receive a ack package during MESSAGE_RESOLVE\r\n");
#endif
