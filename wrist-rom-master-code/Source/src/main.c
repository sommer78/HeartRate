...
static void advertising_init(uint8_t adv_flags)
{
    ...
    **********************************************/
    ...
    uint8_t mac_add_array[4];

    int8_t i = 0;
    for(i = 3; i>=0; --i) {
        mac_add_array[3 - i] =  (mac_address >> 8*i) & 0xff;
    }

    ble_advdata_manuf_data_t adv_manu;
    adv_manu.data.p_data = mac_add_array;
    adv_manu.data.size = 4;
    adv_manu.company_identifier = (((NRF_FICR->DEVICEADDR[1] & 0xff ) << 8) | ((NRF_FICR->DEVICEADDR[1] & 0xFF00) >> 8)); //device info
...
static void dev_loss_alert_handle(int8_t rssi,DEV_LOSS_ALERT_LEVEL alert_level)
{
    ...
    if(get_global_dev_loss_should_alert() == 0 &&  average > DEV_LOSS_RESUME_RSSI_THRESHOLD) {
        set_global_dev_loss_should_alert(1);
        reach_low_time = 0;
    } else  if(get_global_dev_loss_should_alert() == 1 && average <= DEV_LOSS_ALERT_RSSI_THRESHOLD){
        reach_low_time ++;
    }

    if(get_global_dev_loss_should_alert() == 1 && reach_low_time > DEV_LOSS_START_ALARM_COUNT) { //should alert
        switch(alert_level) {
            case NO_ALERT:
                break;
            case MIDDLE_ALERT:
                set_global_dev_loss_should_alert(0); //alert so need to start judge process
                notification_start(NOTIFICATION_LOSE,0);
                break;
            case HIGH_ALERT:
                set_global_dev_loss_should_alert(0);
                notification_start(NOTIFICATION_LOSE,1);
                break;
            default:
                break;
        }
