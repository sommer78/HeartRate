...
uint16_t bond_manager_get_first_bonded_master_id(void) 
{

    return m_masters_db[0].bond.master_id_info.div;
}

//if full, delete the first element.
void ble_bondmngr_bonded_masters_check(void)
{
    if (m_masters_in_db_count == BLE_BONDMNGR_MAX_BONDED_MASTERS){
        uint16_t master_id = m_masters_db[0].bond.master_id_info.div;
        ble_bondmngr_bonded_master_delete(master_id);
    }
}
