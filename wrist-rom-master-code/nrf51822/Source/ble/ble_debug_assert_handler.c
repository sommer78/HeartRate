    ...
    if(address) {
        ...
        if((header & 0xFFFF0000U) == BLE_FLASH_MAGIC_NUMBER) {
            ...
            if(cal_crc == crc_header) {
                ...
                //restore curr_sleep_status to Ram
                check_and_restore(&curr_sleep_status,current_pos,sizeof(uint32_t),end_addr);
                if(curr_sleep_status){
...
static void pre_restart_info_store(backup_info_restart_type_t restart_type)
{
    ...
    curr_sleep_status = (uint32_t)get_curr_sleep_flag();    
    ...
    //store curr_sleep_status to flash
    check_and_backup(current_pos,&curr_sleep_status, sizeof(uint32_t),end_addr);
