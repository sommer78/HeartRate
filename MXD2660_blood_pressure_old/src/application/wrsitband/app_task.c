...
osPoolId  wristband_data_pool_id;
osPoolDef(wristband_data_pool, WRISTBAND_MSG_MAX_NUM, uint8[WRISTBAND_POOL_SIZE]);
...
void app_task_init(void)
{
	...
	wristband_data_pool_id = osPoolCreate(osPool(wristband_data_pool));
        ...
        switch(app_event.status)
        {
            ...
            case osEventMessage:
                ...
                switch(p_msg->msg_module_type)
                {
                    ...
                    case MSG_WRISTBAND:
						//APP_DBG(("task receive msg form cb\n"));
                        wristband_msg_handler(p_msg);
						
                        //release ble message buffer
                        if(p_msg->p != NULL)
                        {
                            osPoolFree(wristband_data_pool_id, p_msg->p);
                        }
                        
                        osPoolFree(app_msg_pool_id, p_msg);
                        break;
                }
                break;
