...
//	  APP_DBG(("size = %d \r\n ",size));	
  ...
  for (i = 0; i < 3000; i++){
		...
		totalPoint++;
		 ...
		 APP_DBG(("%d,",adValues[i]));		  
		// bpmState = 	 getHeartRateWaveState(wave_2660_2[i]);  
		 bpmState = 	 getHeartRateWaveState(adValues[i]);  
...
void hw_timer_config(void)
{
	...
	hw_timer_init(TIME1, TIMER_CLK_8M, 1, TIMER_CHANNEL_0, hw_timer1_handler);
