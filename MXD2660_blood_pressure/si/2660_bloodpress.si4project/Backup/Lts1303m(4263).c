...
#define MAXLEVEL	820
#define MINLEVEL	40
#define MAXTOPLEVEL	1500
#define MAXBOTTOMLEVEL	1500
...
int plusePeakStack[PLUSE_SAMPLE_MAX];
...
int pushPulsePeakStack(int pulsePeakLocation){
	plusePeakStack[pulseIndex]= pulsePeakLocation;
	pulseIndex++;
	return pulseIndex;
}
...
HRState getHeartRateWaveState(uint16_t adData){
	...
	if(isTypeRect){
		if(isDetectStart){
			...
			pushPulsePeakStack(pulsePeakLocal);
			APP_DBG(("pulse   pulsePeakLocal: %d bottomStart : %d bottom: %d\r\n",pulsePeakLocal,bottomStart,bottom));
