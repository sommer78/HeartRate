...
//if use MIC_BIAS as Dmic input, open it 
//#define DMIC_USE_MIC_BIAS
...
void PINMUX_Configuration(void)
{
    ...
    /* dmic pinmux */
    Pinmux_Config(P4_1, DMIC_CLK);
    Pinmux_Config(P4_0, DMIC_DATA);
...
void IO_DemoTask(void* param)
{
    ...
    char* demoStr = "## Please send audio command ##\r"
                    "## 0x01: start AMIC record ##\r"
                    "## 0x02: stop AMIC record ##\r";
    ...
    adcInitStruct.amicEn = ADC_AMIC_Disable;
    adcInitStruct.adcBurstSize        = 0x10;
    adcInitStruct.adcFifoThd          = 0x20;
    
    ADC_Init(ADC, &adcInitStruct);
#ifdef DMIC_USE_MIC_BIAS
    ADC_SetMICBIAS(ADC, ANA_MIC_BIAS_VREF_2_94);
    ADC_Cmd_MICBIAS(ADC, ENABLE);
#endif
    //set audio data little endian
    ...
    codecInitStruct.micType = CODEC_DMIC;
    codecInitStruct.msbcEn  = CODEC_MSBC_DISABLE;   
    codecInitStruct.micVolume =0x4f;    /* 12dB((0x4f - 0x2f)*0.375)*/
...
#if 1      /* print all registers for debug */
                        uint8_t regCount = 0;
