...
**********************************************************************************************************
* @file     main.c
* @brief    ADC battery level detect demo.
* @details  Use ADC one shot mode and compare function to detect battery level,.
*           Use timer to start a ADC convert.
* @author   tifnan
* @date     2015-07-02
* @version  v0.1
*********************************************************************************************************
...
xTimerHandle ADC_StartTimer = NULL;
uint8_t BatLevIsLow = 0;
...
void IO_DemoTask(void* param)
{
    ...
    char* demoTipStr = "### ADC battery level detect demo ###\r\n";
    ...
    ADC_INTConfig(ADC, ADC_INT_CMP_BAT | ADC_INT_ONE_SHOT_DONE, ENABLE);
    /* enable auto sensor mode */
    ADC_Cmd(ADC, ADC_One_Shot_Mode, ENABLE);
    
    /* create sw timer, 10s */
    ADC_StartTimer = xTimerCreate((const char*)"ADC_Timer",\
                        10000 / portTICK_RATE_MS, pdFALSE, NULL, ADC_CollectTimeout);
    /* start timer */
    xTimerStart(ADC_StartTimer, 0);
    ...
    while(1)
    {
        while(xQueueReceive(DemoIOEventQueue, &event, portMAX_DELAY) == pdPASS)
        {
            ...
            if (event == IO_DEMO_EVENT_ADC_CONVERT_DONE) 
            {
                /* voltage value(hex) */
