...
/* added by austin on 2014/06/05 */
/**
 * TimerCallbackFunction_t pvTimerGetCallbackFunc( TimerHandle_t xTimer );
 *
 * Returns the callback function assigned to the timer.
 *
 * The callback functions are assigned to timers using the pxCallbackFunction
 * parameter of the call to xTimerCreated() that was used to create the timer.
 *
 *
 * @param xTimer The timer being queried.
 *
 * @return The The callback function assigned to the timer being queried.
 *
 * Example usage:
 *
 */

TimerCallbackFunction_t pvTimerGetCallbackFunc( const TimerHandle_t xTimer )  PRIVILEGED_FUNCTION;
