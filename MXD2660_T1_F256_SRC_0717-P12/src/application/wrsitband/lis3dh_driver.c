...
uint8_t SPI_Mems_Read_Reg(uint8 reg,uint8 *data)
{
	uint8 tx_buf[2] = {0};
	uint8 rx_buf[2] = {0};
	reg = reg | 0x80;
	tx_buf[0] = reg;
	tx_buf[1] = 0x55;
	spi0_master_trx(0x0202, tx_buf, rx_buf);
	data[0] = rx_buf[1];
	return 1;
}
void SPI_Mems_Write_Reg(uint8_t reg, uint8_t Data)
{
	uint8 tx_buf[2] = {0};
	reg = reg & 0x7F;
	tx_buf[0] = reg;
	tx_buf[1] = Data;
	//transfer_len  [0-7]send length [8£º15]receive length
	spi0_master_rx_only(0x0002,tx_buf);
	//spi_master_tx_only(0x0202,tx_buf);
}
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
* Return		: None
*******************************************************************************/
uint8_t LIS3DH_ReadReg(uint8_t Reg, uint8* Data) {
  
  //To be completed with either I2c or SPI reading function
  SPI_Mems_Read_Reg(Reg, Data);  
  //*Data = read1byte(Reg);
  return 1;
}
...
void LIS3DH_InitSPIDrive(void)
{
	spi0_master_init(TRX_DUPLEX, CPOL_CPHA_1_1, SPI_RATE_2M);
...
uint32 LIS3DH_cmd_proc(void *buffer)
{
	uint8 read_data[4] = {0};
	...
	if(0x01 == ptr[0])
	{
		LIS3DH_ReadReg(ptr[1],&read_data[2]);
		read_data[0] = WRISTBAND_CMD_3DH;
		read_data[1] = 1;
		wristband_notify_AppSendToBle((WRISTBAND_CMD_HEAD *)read_data);
	}
	else if(0x02 == ptr[0])
	{
		LIS3DH_WriteReg(ptr[2],ptr[3]);
	}
	return 2;
}
